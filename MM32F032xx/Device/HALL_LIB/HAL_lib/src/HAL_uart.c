...
void UART_Init(UART_TypeDef* UARTx, UART_InitTypeDef* UART_InitStruct)
{
    ...
    tmpreg |=
        (uint32_t)UART_InitStruct->UART_WordLength | (uint32_t)UART_InitStruct->UART_StopBits | UART_InitStruct->UART_Parity;
...
void UART_Init(UART_TypeDef* UARTx, UART_InitTypeDef* UART_InitStruct)
{
    ...
    /* Determine the UART_baud*/
    tmpreg = (apbclock * 10 / 16) / (UART_InitStruct->UART_BaudRate);
    if ((tmpreg % 10) > 4) {
        tmpreg += 10;
    }
    tmpreg = tmpreg / 10;
