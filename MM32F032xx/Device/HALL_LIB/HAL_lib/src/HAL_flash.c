...
FLASH_Status FLASH_ProByteDataConfig(uint32_t SelectProtectRegion_x, uint16_t Size_x_K, uint16_t Start_x_K)
{
    u32          tempAddH, tempAddL;
    u16          tempStartH, tempStartL, tempSizeH, tempSizeL;
    FLASH_Status status = FLASH_COMPLETE;

    tempAddL   = SelectProtectRegion_x * 4 + 0x1FFE0000;
    tempAddH   = tempAddL + 16;
    tempStartL = ((Start_x_K)&0xff) | (((~Start_x_K) << 8) & 0xff00);
    tempStartH = ((Start_x_K >> 8) & 0xff) | (((~(Start_x_K >> 8)) << 8) & 0xff00);
    tempSizeL  = ((Size_x_K)&0xff) | (((~Size_x_K) << 8) & 0xff00);
    tempSizeH  = ((Size_x_K >> 8) & 0xff) | (((~(Size_x_K >> 8)) << 8) & 0xff00);

    FLASH_ProgramSysSecProByteData(tempAddL, tempSizeL);
    FLASH_ProgramSysSecProByteData(tempAddL + 2, tempStartL);
    FLASH_ProgramSysSecProByteData(tempAddH, tempSizeH);
    status = FLASH_ProgramSysSecProByteData(tempAddH + 2, tempStartH);

    /* Return the Option Byte Data Program Status */
    return status;
}

/*read spec add ,return half word*/
...
{
    u32 wrAdress = 0x1FFE1000 + offset;
    while (len--) {
        *buf++ = *(u16*)wrAdress;
        wrAdress += 2;
    }
...
FLASH_Status FLASH_SysSecWriteGroup(uint32_t offset, u16* buf, u16 len)
{
    ...
    FLASH_Unlock();
    while (len--) {
        status = FLASH_ProgramSysSecProByteData(wrAdress, *buf++);
        wrAdress += 2;
    }
    return status;
}
...
FLASH_Status FLASH_SysSecSetKey(u32 keyH, u32 keyL)
{
    FLASH_Status status = FLASH_COMPLETE;
    FLASH_Unlock();
    FLASH_EraseSecSpace(0x1FFE1000);
    FLASH_EraseSecSpace(0x1FFE1B00);
    status = FLASH_ProgramSysSecProByteData(0x1FFE1000 + 1024 * 3 - 8, keyL & 0XFFFF);
    status = FLASH_ProgramSysSecProByteData(0x1FFE1000 + 1024 * 3 - 6, (keyL >> 16) & 0XFFFF);
    status = FLASH_ProgramSysSecProByteData(0x1FFE1000 + 1024 * 3 - 4, keyH & 0XFFFF);
    status = FLASH_ProgramSysSecProByteData(0x1FFE1000 + 1024 * 3 - 2, (keyH >> 16) & 0XFFFF);
