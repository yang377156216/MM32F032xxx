...
void CAN_Peli_FilterInit(CAN_Peli_FilterInitTypeDef* CAN_Peli_FilterInitStruct)
{
    ...
    CAN1_PELI->SFF   = CAN_Peli_FilterInitStruct->CAN_FilterId0;
...
void CAN_Peli_Transmit(CanPeliTxMsg* PeliTxMessage)
{
    ...
    CAN1_PELI->SFF = (PeliTxMessage->FF << 7) | (PeliTxMessage->RTR << 6) | (PeliTxMessage->DLC);
...
{
    uint32_t tempid;
    PeliRxMessage->FF  = (CAN1_PELI->SFF) >> 7;
    PeliRxMessage->RTR = ((CAN1_PELI->SFF) >> 6) & 0x1;
    PeliRxMessage->DLC = (CAN1_PELI->SFF) & 0xf;
