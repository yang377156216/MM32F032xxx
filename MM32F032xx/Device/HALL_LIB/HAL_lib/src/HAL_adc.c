...
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
    ...
    tmpreg1 |=
        ((uint32_t)ADC_InitStruct->ADC_DataAlign) | ADC_InitStruct->ADC_ExternalTrigConv | ((uint32_t)ADC_InitStruct->ADC_Mode);

    //	tmpreg1 |= ((uint32_t)ADC_InitStruct->ADC_DataAlign) |
    //            ((uint32_t)ADC_InitStruct->ADC_Mode) | ((uint32_t)ADC_InitStruct->ADC_ContinuousConvMode);
...
/**
 * @brief Reset all ADCx channel.
 * @param ADCx: where x can be 1, 2 to select the ADC peripheral.
 * @retval :
 */
void ADC_RegularChannelDisableAll(ADC_TypeDef* ADCx)
{
    ADCx->ADCHS = 0;
}
/**
